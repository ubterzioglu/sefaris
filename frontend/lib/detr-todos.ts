...
/**
 * DETR data lives in the ubt2026 Supabase project — its own env vars keep it
 * separate from the sefarisis waitlist project (NEXT_PUBLIC_SUPABASE_URL).
 * Falls back to the shared NEXT_PUBLIC_* vars only if the dedicated ones are
 * absent, so a single-project setup still works.
 */
function getServiceEnv() {
  const url =
    process.env.UBT_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceRoleKey =
    process.env.UBT_SUPABASE_SERVICE_ROLE_KEY ||
